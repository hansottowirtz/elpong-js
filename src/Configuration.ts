module Elpong {
  export class SchemeConfiguration {
    ...
    constructor(preconf: any) {
      ...
      if ((typeof DEBUG !== 'undefined' && DEBUG) && !this.name) {
        throw new ElpongError('confnn');
      }
      this.selector = preconf.selector;
      if ((typeof DEBUG !== 'undefined' && DEBUG) && !this.selector) {
      ...
      for (let collection_name in preconf.collections) {
        ...
        let collection_configuration: CollectionConfiguration = collection_preconf = {
          singular: collection_preconf.singular || collection_preconf.name.slice(0, -1)
        };
        ...
        for (let relation_type in relation_types) {
