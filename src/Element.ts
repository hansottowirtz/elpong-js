module Elpong {
  ...
  export class Element {
    ...
    constructor(collection: Collection, pre_element: PreElement) {
      ...
      ElementHelper.Fields.setup(this, collection_config.fields, pre_element);
      ElementHelper.Relations.setup(this, collection_config.relations);
      ElementHelper.Actions.setup(this, collection_config.actions);
    ...
    selector(): SelectorValue|undefined {
      return this.fields[this.collection().scheme().selector_name];
    }
      ...
      Util.forEach(collection_settings.fields, (field_settings, field_name) => {
        ...
        if (field_value = pre_element[field_name]) {
          if (field_settings.embedded_element) {
            ElementHelper.Fields.handleEmbeddedElement(this, pre_element, field_name, field_settings);
          } else if (field_settings.embedded_collection) {
            ElementHelper.Fields.handleEmbeddedCollection(this, pre_element, field_name, field_settings);
          } else {
